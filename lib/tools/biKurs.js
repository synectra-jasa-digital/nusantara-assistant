...
import { proxiedFetch } from '../proxyFetch.js'
...
async function callSoap(operation, paramsXml = '') {
  const res = await proxiedFetch(SOAP_URL, {
    method: 'POST',
    headers: {
...
function parseDiffgramRows(parsed, resultKey) {
  ...
  if (!result) {
    throw new Error(
      'Respons dari BI bukan format SOAP yang diharapkan - kemungkinan besar diblokir WAF/firewall BI untuk IP datacenter (lihat README bagian Catatan & Keterbatasan), bukan masalah nama field.'
    )
  }
