...
export async function getRegionInfo({ query }) {
  ...
  // Query only named the city/regency (the common case, e.g. "Jakarta
  // Pusat") - report at that level, since that's the level the user
  // asked about. get_weather still needs an adm4 code regardless, so
  // resolve one representative village under it and expose that
  // separately as weather_code rather than overwriting code/name/level.
  if (!districtMatch) {
    const repDistrict = districts[0]
    const repVillages = repDistrict ? (await fetchJson(`${BASE}/villages/${repDistrict.code}.json`)).data : []
    return {
  ...
  const villageMatch = findByName(villages, query)

  // Query named the kecamatan specifically, not a village within it -
  // report at kecamatan level, same reasoning as above.
  if (!villageMatch) {
    return {
      ...
      weather_code: villages[0]?.code ?? null,
    }
  }
